...
class MultiTrainer(BasicTrainer):
    ...
    def affine_transformation(
        ...
        if "Affine" in self.models:
            if self.model_config.Affine.type == 'models.modules.AffineTransform':
                affine_trs = self.models['Affine'](image_infos)
                rgb_transformed = (affine_trs[..., :3, :3] @ rgb_blended[..., None] + affine_trs[..., :3, 3:])[..., 0]
            elif self.model_config.Affine.type == 'models.modules.BilateralAffineTransform':
                affine_trs = self.models['Affine'](rgb_blended, image_infos)
                affine_trs = affine_trs.reshape(rgb_blended.shape[0], rgb_blended.shape[1], 3, 4)
                rgb_transformed = (affine_trs[..., :3, :3] @ rgb_blended[..., None] + affine_trs[..., :3, 3:])[..., 0]
            elif self.model_config.Affine.type == 'models.modules.NeuralBilateralAffineTransform':
                affine_trs = self.models['Affine'](rgb_blended, image_infos)
                affine_trs = affine_trs.reshape(rgb_blended.shape[0], rgb_blended.shape[1], 3, 4)
                rgb_transformed = (affine_trs[..., :3, :3] @ rgb_blended[..., None] + affine_trs[..., :3, 3:])[..., 0] + rgb_blended
            elif self.model_config.Affine.type == 'models.modules.MultiScaleNeuralBilateralAffineTransform':
                affine_trs = self.models['Affine'](rgb_blended, image_infos)
                affine_trs = affine_trs.reshape(rgb_blended.shape[0], rgb_blended.shape[1], 3, 4)
                rgb_transformed = (affine_trs[..., :3, :3] @ rgb_blended[..., None] + affine_trs[..., :3, 3:])[..., 0] + rgb_blended
            elif self.model_config.Affine.type == 'models.modules.MultiAffineTransform':
                affine_trs = self.models['Affine'](rgb_blended, image_infos)
                if int(image_infos["img_idx"][0][0]) % 3 == 0:
                    affine_trs = affine_trs.reshape(rgb_blended.shape[0], rgb_blended.shape[1], 3, 4)
                rgb_transformed = (affine_trs[..., :3, :3] @ rgb_blended[..., None] + affine_trs[..., :3, 3:])[..., 0]
            elif self.model_config.Affine.type == 'models.modules.MultiScaleBilateralAffineTransform':
                affine_trs = self.models['Affine'](rgb_blended, image_infos)
                rgb_transformed = rgb_blended
                for aff in affine_trs:
                    aff = aff.reshape(rgb_blended.shape[0], rgb_blended.shape[1], 3, 4)
                    rgb_transformed = (aff[..., :3, :3] @ rgb_transformed[..., None] + aff[..., :3, 3:])[..., 0]
