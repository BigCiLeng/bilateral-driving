...
class RigidNodes(VanillaGaussians):
    ...
    # change
    def calculate_rotation_quaternion(self, rotation_angle):
        
        theta_x = rotation_angle[0]
        theta_y = rotation_angle[1]
        theta_z = rotation_angle[2]

        
        qw = torch.cos(theta_z / 2) * torch.cos(theta_y / 2) * torch.cos(theta_x / 2) + \
            torch.sin(theta_z / 2) * torch.sin(theta_y / 2) * torch.sin(theta_x / 2)

        qx = torch.sin(theta_z / 2) * torch.cos(theta_y / 2) * torch.cos(theta_x / 2) - \
            torch.cos(theta_z / 2) * torch.sin(theta_y / 2) * torch.sin(theta_x / 2)

        qy = torch.cos(theta_z / 2) * torch.sin(theta_y / 2) * torch.cos(theta_x / 2) + \
            torch.sin(theta_z / 2) * torch.cos(theta_y / 2) * torch.sin(theta_x / 2)

        qz = torch.cos(theta_z / 2) * torch.cos(theta_y / 2) * torch.sin(theta_x / 2) - \
            torch.sin(theta_z / 2) * torch.sin(theta_y / 2) * torch.cos(theta_x / 2)
    ...
    def create_from_pcd(self, instance_pts_dict: Dict[str, torch.Tensor]) -> None:
        ...
        # pose refinement
        # move vehicles or pedestrians, try to modify the values of instances_quats and instances_trans
        ...
        # # change
        # translation_vector = torch.tensor([[0.0, 0.0, 5.0]], device=self.device)
        # translation_vector = translation_vector.expand(instances_trans.size(0), -1, -1)
        # self.instances_trans = Parameter(instances_trans + translation_vector)
        
        # self.instances_quats = Parameter(self.quat_act(instances_quats)) # (num_frame, num_instances, 4)
        # # rotation_angle = torch.tensor([0.0, 0.0, np.pi / 4], device=self.device)
        # # new_quat = self.calculate_rotation_quaternion(rotation_angle)
        # # self.instances_quats = Parameter(self.quat_multiply(self.instances_quats, new_quat))
    ...
    # debug
    def print_instance_info(instance_pts_dict):
        for id_in_dataset, attributes in instance_pts_dict.items():
            print(f"Instance ID: {id_in_dataset}")
            for attr_name, attr_value in attributes.items():
                if isinstance(attr_value, torch.Tensor):
                    print(f"  {attr_name}: Tensor of shape {attr_value.shape}")
                else:
