...
from tools.eval_metrics import do_evaluation
...
def main(args):
    ...
    for step in metric_logger.log_every(all_iters, cfg.logging.print_freq):
                ...
                wandb.log(
                    {
                        ...
                        "geo_metrics/depth_err_median_squared": render_results["depth_err_median_squared"],
                        "geo_metrics/avg_chamfer": render_results["avg_chamfer"],
                        "geo_metrics/RMSE": render_results["depth_err"],
                    }
                )
        ...
        do_save = step > 0 and (
            (step % cfg.logging.saveckpt_freq == 0) or
            # (step % 10000 == 0) or
            (step == trainer.num_iters)
        ) and (args.resume_from is None)
