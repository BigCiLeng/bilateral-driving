...
class BasicTrainer(nn.Module):
    ...
    def compute_losses(
        ...
        if affine_reg is not None and "Affine" in self.models:
            if self.model_config.Affine.type == 'models.modules.BilateralAffineTransform':
                loss_affine = self.models['Affine'].tv_loss()
                loss_dict.update({
                    "affine_loss": affine_reg.w * loss_affine
                })
            elif self.model_config.Affine.type == 'models.modules.AffineTransform':
            ...
            elif self.model_config.Affine.type == 'models.modules.MultiTransform':
                ...
                if img_idx % 3 == 0:
                    ...
                    affine_trs = self.models['Affine'](predicted_rgb, {"img_idx": image_infos["img_idx"].flatten()[0]})
                    reg_mat = torch.eye(3, device=self.device)
                    reg_shift = torch.zeros(3, device=self.device)
                    loss_affine = torch.abs(affine_trs[..., :3, :3] - reg_mat).mean() + torch.abs(affine_trs[..., :3, 3:] - reg_shift).mean()
                    loss_dict.update({
