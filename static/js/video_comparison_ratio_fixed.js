// Video Comparison Functions with Correct Display Ratios
// 2-way comparison: Canvas display width = video display width / 2
// 3-way comparison: Canvas display width = video display width / 3
...
function playVids(videoId) {
    ...
    // Canvas display size should match the video's display size for 2-way comparison
    // Height is scaled by 2 for Rendering Results Comparison - use actual scaled height
    const scaledHeight = videoRect.height * 2;
    videoMerge.style.width = videoRect.width + 'px';
    videoMerge.style.height = scaledHeight + 'px';
    
    // Update container height to match scaled video
    const container = videoMerge.closest('.single-video-container');
    if (container) {
...
function playVids3(videoId) {
    ...
    let dragIndex = -1;
    const vidWidth = vid.videoWidth / 3;  // Canvas width is one third of video width for 3-way comparison
    const vidHeight = vid.videoHeight;
    ...
    videoMerge.width = vidWidth;
    videoMerge.height = vidHeight;
    // Canvas display size should match the video's display size for 3-way comparison
    // Height is scaled by 3 for Teaser Video - use actual scaled height
    const scaledHeight = videoRect.height * 3;
    videoMerge.style.width = videoRect.width + 'px';
    videoMerge.style.height = scaledHeight + 'px';
    
    // Update container height to match scaled video
    const container = videoMerge.closest('.teaser-video');
    if (container) {
    ...
    function drawLoop() {
        ...
        // Draw first segment (left video) - use first third of the source video
        const thirdWidth = vidWidth;
        mergeContext.drawImage(vid, 0, 0, thirdWidth, vidHeight, 0, 0, thirdWidth, vidHeight);
        
        // Draw second segment (middle video)
        const colStart1 = (thirdWidth * positions[0]).clamp(0, thirdWidth);
        const colWidth1 = ((thirdWidth * positions[1]) - (thirdWidth * positions[0])).clamp(0, thirdWidth);
        mergeContext.drawImage(vid, colStart1 + thirdWidth, 0, colWidth1, vidHeight, colStart1, 0, colWidth1, vidHeight);
        
        // Draw third segment (right video)
        const colStart2 = (thirdWidth * positions[1]).clamp(0, thirdWidth);
        const colWidth2 = (thirdWidth - (thirdWidth * positions[1])).clamp(0, thirdWidth);
        mergeContext.drawImage(vid, colStart2 + thirdWidth * 2, 0, colWidth2, vidHeight, colStart2, 0, colWidth2, vidHeight);
        
        // Draw divider lines
        const lineX1 = thirdWidth * positions[0];
        const lineX2 = thirdWidth * positions[1];
