...
class DrivingDataset(SceneDataset):
    ...
    @property
    def training_indices_for_test(self):
        num_cams = self.pixel_source.num_cams
        num_images = self.num_img_timesteps * num_cams
        full_list = {}
        for idx in self.test_indices:
            near_list = []
            prev_index = (idx - num_cams) if (idx - num_cams) >= 0 else None
            next_index = (idx + num_cams) if (idx + num_cams) < num_images else None
            if prev_index is not None and prev_index in self.train_indices:
                # near_list.append(self.train_indices.index(prev_index))
                near_list.append(prev_index)
            if next_index is not None and next_index in self.train_indices:
                near_list.append(next_index)
            full_list[idx] = near_list
        return full_list
